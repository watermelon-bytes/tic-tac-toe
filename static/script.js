...
function drawField(sideOfField) {
    ...
    for (let i = 1; i <= sideOfField ** 2; i++) {
        ...
        cell.style.left = `$ { (column - 1) * cellSize
        }
        px`;
...
let setChoice = function() {
    ...
    return new Promise((resolve) => {
        ...
        ['x', 'o'].forEach((elem) => {
            let button = createChoiceButton(`$ {
                elem
            }
            _btn`, promptText + elem.toUpperCase());
...
function getComputerMove(session) {
    let request = fetch('/game', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json;charset=utf-8'
        },
